	...
	if (this.path) {
		if (this.path.length==0) {
			...
			var action = this.curveFlag ? 
					...
					this.sketchbook.addLineAction(this.sketchId, this.path, this.frameStyle,this.circleStyle, this.lineColor, this.fillColor);
			this.path.remove();
			return action;
		}
	}
...
function CircleTool(project, sketchbook, sketchId, description) {
	Tool.call(this, 'circle', project);
	this.sketchbook = sketchbook;
	this.sketchId = sketchId;	
	this.description = description;
	this.circleStyle = getProperty('circleStyle', 'border');
	this.lineColor = getLineColor();
	this.fillColor = getFillColor();
	this.lineWidth = getProperty('lineWidth', 1);
	this.showLabel = getProperty('showLabel', 'circle');
	this.textColor = getTextColor();
	this.textSize = getProperty('textSize', 12);
	this.textVAlign = getProperty('textVAlign', 'middle');
}
...
CircleTool.prototype.end = function(point) {
	...
	var bounds = new paper.Rectangle(this.startPoint, point);
	return this.sketchbook.addCircleAction(this.sketchId, this.description, bounds, this.circleStyle, this.lineColor, this.lineWidth, this.fillColor, this.showLabel, this.textColor, this.textSize, this.textVAlign);
};
